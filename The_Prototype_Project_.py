...
# Função para obter informações da creche
...
def cadastrar(tipo):
    if validar_cpf(tipo) and validar_email(tipo) and validar_nome(tipo) and validar_idade(tipo):
        nome = entry_nome.get() if tipo == 'aluno' else entry_nome_prof.get()
        email = entry_email.get() if tipo == 'aluno' else entry_email_prof.get()
        idade = int(entry_idade.get()) if tipo == 'aluno' else int(entry_idade_prof.get())
        cpf = entry_cpf.get() if tipo == 'aluno' else entry_cpf_prof.get()
        ...
        if tipo == 'professor':
            ...
            cursor.execute(f"INSERT INTO {tabela} (nome, email, cpf, idade, registro, salario) VALUES (%s, %s, %s, %s, %s, %s)", 
                           (nome, email, cpf, idade, registro, salario))
        else:
            telefone = entry_telefone.get()
            cursor.execute(f"INSERT INTO {tabela} (nome, email, cpf, idade, registro, telefone_dos_responsaveis) VALUES (%s, %s, %s, %s, %s, %s)", 
                           (nome, email, cpf, idade, registro, telefone))
        db.commit()
...
def gerar_pdf():
    ...
    # Adicione as informações da creche no PDF
